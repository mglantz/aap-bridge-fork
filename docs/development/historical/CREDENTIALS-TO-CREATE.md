...
```bash
# Get credential IDs
ANSIBLE_GALAXY_ID=$(curl -sk -H "Authorization: Bearer YOUR_TARGET_AAP_TOKEN" \
  "https://localhost:10443/api/controller/v2/credentials/?name=Ansible+Galaxy" \
  | jq -r '.results[0].id')

HUB_VALIDATED_ID=$(curl -sk -H "Authorization: Bearer YOUR_TARGET_AAP_TOKEN" \
...
```bash
# Get project ID
PROJECT_ID=$(curl -sk -H "Authorization: Bearer YOUR_TARGET_AAP_TOKEN" \
...
```bash
# Get job template ID
TEMPLATE_ID=$(curl -sk -H "Authorization: Bearer YOUR_TARGET_AAP_TOKEN" \
